...
const paths = {
  ...
  persistencePublicApi: 'editor/persistence/src/public-api.ts',
  persistenceDomainFile: 'editor/persistence/src/domain/file.ts',
  persistenceSnapshotService: 'editor/persistence/src/application/snapshot-service.ts',
  persistenceSnapshotServiceTest: 'editor/persistence/src/application/snapshot-service.test.ts',
  lifecycleTest:
...
function isFinalState() {
  ...
  if (!existsSync(abs(paths.editorSession))) return false
  if (!existsSync(abs(paths.canvasDocumentService))) return false
  if (!existsSync(abs(paths.fileGateway))) return false
  if (!existsSync(abs(paths.persistenceDomainFile))) return false

  const document = read(paths.document)
  const editorSession = read(paths.editorSession)
  const canvasDocumentService = read(paths.canvasDocumentService)
  const fileGateway = read(paths.fileGateway)
  const persistenceDomainFile = read(paths.persistenceDomainFile)
  ...
  return (
    ...
    !persistenceDomainFile.includes('TLEditorSnapshot')
  )
}
...
function patchPersistenceTypes() {
  write(
    paths.persistencePublicApi,
...
`,
  ...
  removeIfExists(paths.persistenceSnapshotService)
  removeIfExists(paths.persistenceSnapshotServiceTest)
}
...
function finalValidate() {
  ...
  if (editorSession.includes('captureLegacyEditorSnapshot')) {
    throw new Error('Obsolete legacy editor snapshot bridge remains')
  }
  if (canvasDocumentService.includes('serializeDrawDocument')) {
