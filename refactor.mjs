...
const removedFeatures = ['flowchart', 'freehand', 'scientific-plot', 'import-export']
...
const staleReferences = await findStaleReferences()

if (staleReferences.length > 0) {
  console.error('')
  console.error('仍发现已删除 Feature 的引用：')
  ...
  console.log('')
  console.log('无用自定义 Feature 已删除干净。')
  console.log('')
  console.log('已删除：')
  console.log('  - features/flowchart')
  console.log('  - features/freehand')
  console.log('  - features/scientific-plot')
  console.log('  - features/import-export')
  console.log('  - 对应桌面依赖')
  console.log('  - 对应 Extension 注册')
  console.log('  - 对应架构脚手架声明')
  console.log('  - 对应 pnpm lockfile 内容')
  console.log('  - 无调用方的 Inspector Section API')
  console.log('')
  console.log('保留：')
  console.log('  - features/workspace')
  console.log('  - features/settings')
  console.log('  - 官方 tldraw 工具和 Shape')
  console.log('  - 当前右侧属性侧边栏')
  console.log('')
  console.log('验证：')
...
async function cleanApplicationRuntime() {
  await update('apps/desktop/src/bootstrap/application.ts', (source) => {
...
async function cleanDesktopManifest() {
  await updateJson('apps/desktop/package.json', (manifest) => {
...
async function cleanRootManifest() {
  await updateJson('package.json', (manifest) => {
    manifest.description = 'A local-first canvas application built on tldraw.'
...
async function cleanArchitectureScaffolds() {
  await updateJson('architecture.scaffolds.json', (manifest) => {
    manifest.scaffolds = manifest.scaffolds.filter(
      (scaffold) => !removedFeatures.some((feature) => scaffold.path === 'features/' + feature),
    )
...
async function cleanArchitectureRules() {
  await update('tests/architecture/check.mjs', (source) => {
    source = source.replace(
...
async function cleanWorkspaceCatalog() {
  await update('pnpm-workspace.yaml', (source) => {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      const matchingPackage = catalogPackages.find((packageName) =>
        line
          .trimStart()
          .startsWith(packageName.includes('@') ? `"${packageName}":` : packageName + ':'),
      )
...
`
  ...
  await write('editor/core/src/contracts/extension-contract.ts', content)
}
...
async function cleanExtensionPublicExports() {
  for (const relativePath of [
    ...
    await update(relativePath, (source) =>
      source
        .split('\n')
        .filter((line) => !line.includes('HybridCanvasInspectorSection'))
        .join('\n'),
    )
  }
}
...
async function cleanEditorCanvas() {
  await update('editor/core/src/react/EditorCanvas.tsx', (source) => {
    source = source.replace(
      `                inspectorSections={
...
async function cleanInspectorPortal() {
  await update('editor/core/src/react/canvas-inspector-portal.tsx', (source) => {
    source = source.replace(
      `import type {
...
async function cleanPropertiesContent() {
  await update('editor/core/src/react/PropertiesInspectorContent.tsx', (source) => {
    source = source.replace(
...
async function cleanPropertiesArchitectureTest() {
  await update('tests/architecture/check-properties-inspector-architecture.mjs', (source) => {
    source = source.replace(
      `
requirePattern(
  'ExtensionContract',
...
async function removeFeatureDirectories() {
  for (const feature of removedFeatures) {
    await rm(path.join(root, 'features', feature), {
      recursive: true,
      force: true,
    })
  }
...
async function regenerateLockfile() {
  console.log('')
  console.log('正在重新生成 pnpm-lock.yaml...')
  ...
  if (result.status !== 0) {
    throw new Error('PNPM_LOCKFILE_REGENERATION_FAILED')
  }
}
...
async function findStaleReferences() {
  ...
  await scanDirectory(root, needles, ignoredDirectories, matches)

  return matches
}

async function scanDirectory(directory, needles, ignoredDirectories, matches) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await scanDirectory(entryPath, needles, ignoredDirectories, matches)
    ...
    if (!/\.(?:json|yaml|yml|ts|tsx|js|jsx|mjs|cjs|md|css|toml)$/.test(entry.name)) {
      continue
    }

    const content = normalize(await readFile(entryPath, 'utf8'))

    const found = needles.filter((needle) => content.includes(needle))

    if (found.length > 0) {
      matches.push(path.relative(root, entryPath) + ': ' + found.join(', '))
    }
  }
}
...
async function update(relativePath, transform) {
  ...
  const previous = normalize(await readFile(filePath, 'utf8'))
...
async function write(relativePath, content) {
  await writeFile(path.join(root, relativePath), normalize(content).trimEnd() + '\n', 'utf8')
}
