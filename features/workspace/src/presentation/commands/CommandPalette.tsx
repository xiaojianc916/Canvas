...
import { Command, Search } from '@mynaui/icons-react'
...
export function CommandPalette({ open, registry, onOpenChange }: CommandPaletteProps) {
  ...
  const executeCommand = (command: RegisteredCommand) => {
    onOpenChange(false)
    ...
    >
      <div className={['flex items-center gap-2', 'border-b', 'border-divider', 'px-4'].join(' ')}>
        ...
        <Input
          ...
          aria-expanded={open}
          aria-label="搜索命令"
          className={['h-12 border-0', 'px-0 shadow-none', 'focus-visible:ring-0'].join(' ')}
          onChange={(event) => {
        ...
        {filteredCommands.length > 0 ? (
          filteredCommands.map((command, index) => (
            ...
            >
              <Command
                aria-hidden="true"
                className={['size-4', 'text-muted-foreground'].join(' ')}
              />

              <span className={['min-w-0 flex-1', 'truncate'].join(' ')}>{command.label}</span>

              {command.category ? (
                <span className={['text-xs', 'text-muted-foreground'].join(' ')}>
                  {command.category}
                </span>
              ) : null}

              {command.shortcut ? (
                <kbd className={['text-xs', 'text-muted-foreground'].join(' ')}>
                  {command.shortcut}
                </kbd>
              ) : null}
            </button>
          ...
          <EmptyState description="尝试输入其他命令名称或分类。" title="没有匹配的命令" />
        )}
      </div>
    </Dialog>
  )
