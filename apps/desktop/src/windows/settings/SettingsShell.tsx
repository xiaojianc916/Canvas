...
export function SettingsDialog({ open, onOpenChange }: SettingsDialogProps) {
  ...
  return (
    ...
    >
      <div
        className="flex h-[min(720px,88vh)] w-[min(980px,92vw)] overflow-hidden rounded-2xl border bg-background text-foreground shadow-2xl"
        ref={dialogRef}
      >
        <nav aria-label="设置分类" className="w-56 shrink-0 border-r bg-sidebar p-3">
          <div className="mb-5 flex items-center gap-2 px-2 py-1">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">Hybrid Canvas</p>
            </div>
          </div>
        ...
        <section className="min-w-0 flex-1 overflow-y-auto">
          <header className="sticky top-0 z-10 flex h-14 items-center border-b bg-background/95 px-6 backdrop-blur">
            <div>
              <h2 className="text-sm font-semibold">外观与体验</h2>
              <p className="text-[11px] text-muted-foreground">调整应用主题和工作区行为</p>
            </div>
            <Button
              aria-label="关闭设置"
          ...
          <div className="mx-auto max-w-2xl space-y-8 p-6">
            <SettingsSection description="选择适合当前环境的界面外观。" title="外观">
              <SettingsRow description="自动跟随操作系统的浅色或深色设置。" label="跟随系统主题">
                <Switch defaultChecked />
              </SettingsRow>
              <Separator />
              <SettingsRow description="降低面板和弹出层的视觉动态效果。" label="减少动态效果">
                <Switch />
              </SettingsRow>
            </SettingsSection>
            <SettingsSection description="控制工作台在启动和编辑过程中的行为。" title="工作区">
              <SettingsRow description="重新打开上次退出时仍在编辑的文档。" label="恢复上次会话">
                <Switch defaultChecked />
              </SettingsRow>
              <Separator />
              <SettingsRow description="编辑过程中定期将文档写入本地文件。" label="自动保存">
                <Switch defaultChecked />
              </SettingsRow>
            </SettingsSection>
            <div className="rounded-xl border bg-muted/20 p-4 text-[11px] leading-5 text-muted-foreground">
              设置界面已收敛到主窗口模态层。具体选项将在领域设置端口接通后持久化。
            </div>
          </div>
        </section>
      </div>
...
function SettingsNavigationItem({
  ...
  return (
    <button
      ...
      className={`mb-1 flex h-9 w-full items-center gap-2 rounded-lg px-2.5 text-left text-xs ${active ? 'bg-sidebar-accent font-medium text-sidebar-accent-foreground' : 'text-muted-foreground hover:bg-sidebar-accent/60 hover:text-foreground'}`}
      type="button"
    >
      {icon}
      <span>{label}</span>
...
function SettingsSection({
  ...
  return (
    <section>
      ...
      <div className="rounded-xl border bg-surface px-4">{children}</div>
    </section>
  )
}
