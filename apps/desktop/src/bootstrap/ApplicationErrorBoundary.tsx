    ...
    return (
      <main className="grid h-dvh place-items-center bg-background p-8 text-foreground">
        <section className="w-full max-w-lg rounded-2xl border bg-surface p-6 shadow-xl">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">界面发生了未处理错误。错误信息已输出到开发日志，你可以重新加载后继续。</p>
          <pre className="mt-4 max-h-36 overflow-auto rounded-lg bg-muted p-3 text-[11px] leading-5 text-muted-foreground">{error.message}</pre>
          <Button className="mt-5" onClick={() => window.location.reload()} type="button"><RotateCcw className="size-4" />重新加载</Button>
        </section>
      </main>
    )
  }
