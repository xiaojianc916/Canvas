...
const paths = {
  controller: 'apps/desktop/src/fatal/fatal-controller.ts',
  ...
  host: 'apps/desktop/src/fatal/FatalErrorHost.tsx',
}

for (const relativePath of Object.values(paths)) {
  if (!existsSync(path.join(ROOT, relativePath))) {
    failures.push('Missing fatal state-machine file: ' + relativePath)
  }
}
...
if (failures.length === 0) {
  ...
  const collectors = read(paths.collectors)

  const preReact = read(paths.preReact)

  const host = read(paths.host)

  forbidText(controller, 'window.', 'Pure fatal controller depends on window.')

  forbidText(controller, 'import.meta', 'Pure fatal controller depends on Vite.')

  forbidText(controller, 'reactMounted', 'Pure fatal controller owns presentation state.')

  forbidText(controller, 'addEventListener', 'Pure fatal controller installs browser listeners.')
  ...
  requireText(collectors, 'window.addEventListener(', 'Browser fatal collectors are not isolated.')
...
if (failures.length > 0) {
  ...
  console.log('Fatal state-machine architecture checks passed.')
}
