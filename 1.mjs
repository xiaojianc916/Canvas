...
stage(
  'editor/core/src/react/extension-registry.ts',
  `import type {
...
mutateJson('apps/desktop/src-tauri/tauri.conf.json', (value) => {
  value.app.security.csp =
    "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; object-src 'none'; base-uri 'self'; connect-src 'self'; frame-ancestors 'none';"
})

mutateJson('package.json', (value) => {
  value.scripts['test:architecture'] =
    'node tests/architecture/check.mjs'

  value.scripts.check =
    'pnpm test:architecture && turbo run check && pnpm check:rust'
})
...
if (dryRun) {
  console.log('[dry-run] 新建 tests/architecture/check.mjs')
} else {
  const path = join(root, 'tests/architecture/check.mjs')

  mkdirSync(dirname(path), {
    recursive: true,
  })

  writeFileSync(path, architectureTest, 'utf8')
}

if (!dryRun) {
  console.log(`备份目录：${relative(root, backupRoot)}`)
  console.log(
    '运行最小验证：pnpm format && pnpm typecheck && pnpm test:architecture && cargo fmt --all --check && cargo check --workspace --all-targets --all-features',
  )
  ...
  try {
    ...
    execFileSync('pnpm', ['test:architecture'], {
      cwd: root,
      stdio: 'inherit',
    })
    ...
    console.error(
      `回滚：git restore --worktree --staged . && rm -rf ${relative(
        root,
        backupRoot,
      )}`,
    )
